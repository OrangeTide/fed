#!/bin/sh
# Cross-compile an MS-DOS 16-bit (real mode) binary using Open Watcom.
#
...
#   CC       - Watcom C compiler          (default: wcc)
#   HOSTCC   - host C compiler            (default: gcc)
#   BUILDDIR - output directory           (default: build/dos16-watcom)
#
# Note: we deliberately omit -i=. to avoid the project's io.h shadowing
# Watcom's system <io.h>. wcc finds project headers via "" includes
...
CFLAGS="-0 -mh -bt=dos -dTARGET_WATCOM -dTARGET_DOS16 -w4 -ox -zt=256"
...
wlink system dos name "$BUILDDIR/fed.exe" file "$OBJS"
